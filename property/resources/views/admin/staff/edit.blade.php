...
<div class="row form-group">
  <label class="col-md-3 text-right">Nama Panggilan <span class="text-danger">*</span></label>
  <div class="col-md-9">
  <input type="text" name="nickname_staff" class="form-control" placeholder="Nama Panggilan" value="{{ $staff->nickname_staff }}" required>
  </div>
</div>

<div class="row form-group">
<label class="col-md-3 text-right">Nama Lengkap <span class="text-danger">*</span></label>
<div class="col-md-9">
...
<small class="text-success">Telepon/HP (WA)</small>
</div>
</div>
...
<div class="row form-group">
    ...
    <div class="col-md-9">
      <select name="id_provinsi" id="provinsi" class="form-control select2">
        ...
        <option value="{{ $provinsi->id }}" {{ (($staff->id_provinsi==$provinsi->id) ? 'selected' : '') }}>{{ $provinsi->nama }}</option>
        <?php } ?>
      </select>
    </div>
  </div>
    ...
    <div class="col-md-9">
      <select name="id_kabupaten" id="kabupaten" class="form-control select2">
        <?php foreach($kabupaten as $kabupaten) { ?>
          <option value="{{ $kabupaten->id }}" {{ (($staff->id_kabupaten==$kabupaten->id) ? 'selected' : '') }}>{{ $kabupaten->nama }}</option>
        <?php } ?>
      </select>
    </div>
  </div>
    ...
    <div class="col-md-9">
      <select name="id_kecamatan" id="kecamatan" class="form-control select2">
        <?php foreach($kecamatan as $kecamatan) { ?>
          <option value="{{ $kecamatan->id }}" {{ (($staff->id_kecamatan==$kecamatan->id) ? 'selected' : '') }}>{{ $kecamatan->nama }}</option>
        <?php } ?>
      </select>
    </div>
  </div>
